...
module.exports = {
    ...
    goodAll: function (req, res, next) {
        pool.getConnection(function (err, connection) {
            ...
            connection.query($sql.getUserAll, function (err, result) {
                console.log(result.name);
                const data = result[0];
                data.token = 'jkndfkjsai23894owasufhsef80we'
                jsonWrite(res, data);
                // 关闭数据库连接
                connection.release();
            });
    ...
    useradd: function (req, res, next) {
        pool.getConnection(function (err, connection) {
            ...
            connection.query($sql.userinsert, [param.name, param.desc, param.price, param.sum], function (err, result) {
                if (result) {
                    result = {
                        code: 200,
                        msg: '增加成功'
    ...
    userupdate: function (req, res, next) {
        ...
        pool.getConnection(function (err, connection) {
            ...
            // 建立连接，向表中插入值
            // 'INSERT INTO user(id, name, age) VALUES(0,?,?)',
            connection.query($sql.userupdate, [param.name, param.desc, param.price, param.sum, param.id], function (err, result) {
                if (result) {
                    result = {
                        code: 200,
                        msg: '修改成功'
